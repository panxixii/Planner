...
export const DateTimePicker: React.FC<DateTimePickerProps> = ({ id, value, onChange, placeholder = '选择日期和时间', dateOnly = false }) => {
  ...
  const openPicker = () => {
    ...
    if (rect) {
      ...
      const panelHeight = dateOnly ? 380 : 470;
      setPanelPos({
        top: Math.min(Math.max(8, rect.bottom + 6), window.innerHeight - panelHeight - 8),
  ...
  const commit = () => {
    onChange(dateOnly ? formatDate(draftDate) : formatValue(draftDate, draftTime));
    setIsOpen(false);
  };
  ...
  return (
    <div className="relative">
      ...
      {isOpen && panelPos ? createPortal(
        <>
          <button type="button" className="fixed inset-0 z-[180] cursor-default" onClick={() => setIsOpen(false)} aria-label="关闭选择器" />
          <div className="fixed z-[200] w-[330px] rounded-2xl border border-neutral-200 bg-white p-4 shadow-2xl" style={{ top: panelPos.top, left: panelPos.left }}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setIsOpen(false)} className="h-9 rounded-lg border border-neutral-200 px-3 text-xs font-semibold text-neutral-500">取消</button>
              {dateOnly ? (
                <button type="button" onClick={() => { onChange(''); setIsOpen(false); }} className="h-9 rounded-lg border border-neutral-200 px-3 text-xs font-semibold text-neutral-400 hover:bg-neutral-50">清除</button>
              ) : null}
              <button type="button" onClick={commit} className="h-9 rounded-lg bg-purple-600 px-4 text-xs font-semibold text-white hover:bg-purple-700">确定</button>
            </div>
          </div>
        </>,
        document.body,
